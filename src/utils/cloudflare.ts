...
export type CloudflareLocals = App.Locals & Runtime<{ DB: D1Database; COMMENT_HASH_SECRET?: string }>;
...
export function getUploadsBucket(locals: App.Locals) {
  ...
  if (!bucket) throw new Error("Cloudflare R2 binding `UPLOADS` is not available.");
  return bucket;
}
