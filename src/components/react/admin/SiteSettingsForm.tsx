...
export default function SiteSettingsForm() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/settings")
      ...
      .then(data => {
        const result = data as { settings: Partial<SiteSettings> };
        if (mounted) setSettings({ ...emptySettings, ...result.settings });
      })
      .catch(() => {
  ...
  return (
    <form
      onSubmit={onSubmit}
      style={{ ...card, display: "grid", gap: "1.25rem" }}
    >
      <div>
        <h2
          style={{
        ...
        >
          These values drive public SEO tags, RSS metadata, and the header
          brand.
        </p>
      </div>
      ...
      >
        ...
        <label>
          ...
          <select
            ...
            value={settings.dir}
            onChange={event =>
              update("dir", event.target.value as SiteSettings["dir"])
            }
          >
      ...
      <div>
        ...
        >
          These links are rendered on the server, so they do not add client-side
          work to public pages.
        </p>
      </div>
